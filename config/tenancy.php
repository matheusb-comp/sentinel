...
use App\Tenancy\BigintTableRLSManager;
...
return [
    ...
    'models' => [
         ...
         */
        // Null so that Company keeps a plain autoincrement bigint key. The
        // public identifier is companies.slug, which never exposes the id.
        'id_generator' => null,
    ],
    ...
    'identification' => [
        ...
        'resolvers' => [
            ...
            Resolvers\PathTenantResolver::class => [
                'tenant_parameter_name' => 'company',
                // Resolve by slug, never by the numeric tenant key.
                'tenant_model_column' => 'slug',
                'tenant_route_name_prefix' => 'company.',
     ...
     */
    'bootstrappers' => [
        // Reconnects as the RLS role and sets the session variable. Absent from
        // the SQLite suite, which strips it in tests/Pest.php: it issues
        // `SET my.current_tenant = '...'`, which is not valid SQLite.
        ...
        // Deliberately absent:
        //
        // DatabaseTenancyBootstrapper — multi-database only. In `local` and
        // `testing` it throws TenantDatabaseDoesNotExistException when the
        // per-tenant database is missing, which is always our case.
        //
        // DatabaseSessionBootstrapper — a session scoped per tenant would log
        // the user out on every company switch.
        //
        // FilesystemTenancyBootstrapper — no uploads yet; add it when there are.
    ],
    ...
    'rls' => [
         ...
         *
         * A subclass of TableRLSManager, because the tenant key is a bigint and
         * the stock policy casts the column to text, which would make every
         * tenant-scoped query a sequential scan.
         *
         * @see BigintTableRLSManager
         * @see TableRLSManager
         * @see TraitRLSManager
         */
        'manager' => BigintTableRLSManager::class,
    ...
    'cache' => [
        ...
        // The `array` store is excluded because CacheTenancyBootstrapper throws
        // on it, and the test suites run with CACHE_STORE=array. That store is
        // per-process and discarded between tests, so there is nothing to
        // isolate — but it does mean cache scoping is not exercised by tests.
        'stores' => array_values(array_filter(
            [env('CACHE_STORE')],
            fn (?string $store): bool => $store !== null && $store !== 'array',
        )),
